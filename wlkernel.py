...
class WLkernerl():
    ...
    Weisfeiler_Lehman graph kernel.
    """
    def compare_list(self, graph_list, h=1, node_label=False):
        ...
        phi = np.zeros((n_max, n), dtype=np.int64)
        ...
        if node_label is True:
            for i in range(n):
                ...
                # 'node_label'
                labels[i] = np.zeros(len(l_aux), dtype=np.int64)
        ...
        while it < h:
            ...
            phi = np.zeros((n_nodes, n), dtype=np.int64)
            for i in range(n):
                for v in range(len(lists[i])):
                    # form a multiset label of the node v of the i'th graph
                    # and convert it to a string
                    # list: adjacency lists
                    long_label = np.concatenate((np.array([labels[i][v]]),np.sort([i for i in lists[i][v][1]])))
                                                 #np.sort(labels[i]
                                                 #[lists[i][v]])))
                ...
                #np.sum([phi[new_labels[i], i],aux[new_labels[i]],axis=0)
                phi[new_labels[i], i]=[np.int64(i) for i in phi[new_labels[i], i]]
    ...
    def compare(self, g_1, g_2, h=1, node_label=False):
...
if __name__ == "__main__":
    ...
    wlkernel=WLkernerl()
    print(wlkernel.compare(g1,g2,h=10,node_label=False))
